<table class="vis" style="width:100%">
	...
	<tr class="{$villages.$arr_id.lit}">
		...
		<td align="center">{$own_units.$arr_id.$dbname}</td>
			{/if}
		{/foreach}
		<td><a href="game.php?village={$village.id}&amp;screen=place">{$lang->get('praça')}</a></td>
	</tr>
	<tr class="{$villages.$arr_id.lit}units_there">
		...
		<td align="center">{$all_units.$arr_id.$dbname}</td>
			{/if}
		{/foreach}
		<td rowspan="2"><a href="game.php?village={$village.id}&amp;screen=place&amp;mode=units">{$lang->get('tropas')}</a></td>
	</tr>
