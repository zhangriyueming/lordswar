...
if($_GET['action'] == "add_bonus")
	{
	 ...
	 if($anzahl <= 1000 && $anzahl > 1)
		...
		while($er1 = $qu1->fetch(PDO::FETCH_ASSOC))
			{
			 if($bonus == 0)
				{
				 $rand = rand(1, 6);
	...
	$sate1 = ($db->query("SELECT * FROM villages WHERE bonus = '1'"))->rowCount();
$sate2 = ($db->query("SELECT * FROM villages WHERE bonus = '2'"))->rowCount();
$sate3 = ($db->query("SELECT * FROM villages WHERE bonus = '3'"))->rowCount();
$sate4 = ($db->query("SELECT * FROM villages WHERE bonus = '4'"))->rowCount();
$sate5 = ($db->query("SELECT * FROM villages WHERE bonus = '5'"))->rowCount();
$sate6 = ($db->query("SELECT * FROM villages WHERE bonus = '6'"))->rowCount();
...
if($_GET['action'] == "add_villages")
	...
	$sate = ($db->query("SELECT * FROM villages WHERE userid = '-1'"))->rowCount();
$tpl->assign("err", $err);
