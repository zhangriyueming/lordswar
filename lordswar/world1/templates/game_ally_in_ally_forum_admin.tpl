...
{php}
	...
	// die('ok');
	if ($result = $GLOBALS['db']->query("SELECT * FROM forum_structure WHERE ally_id = '".$ally_id."' ORDER BY order_id")) {
		echo "<div>";

		while ($row = $GLOBALS['db']->fetch_assoc($result)) {
...
<br/>
<br/>
<form method="post" action="game.php?village={$village.id}&amp;screen=ally&amp;mode=forum&amp;page=admin&amp;do=newarea">
<table class="vis">
