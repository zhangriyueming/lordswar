		...
		<form action="game.php?village={$village.id}&amp;screen=map&amp;page=mark&amp;action=mark" method="post">
			<td valign="top" width="50%">
				<table class="vis" style="border: 1px solid #804000" align="center" width="100%">
					<tr><th colspan="3">{$lang->get('Marcar jogador')}</th></tr>
					{if !empty($error)}<tr><td colspan="3"><div class="error">{$error}</div></td></tr>{/if}
					<tr>
						<td>&raquo; {$lang->get('Jogador')}:</td>
					...
					<tr><td>&raquo; {$lang->get('Cor')}:</td><td><input type="text" class="color" value="FF0000" maxlength="6" name="color" /></td></tr>
				</table>
			</td>
		</form>
		<td valign="top" width="50%">
			<table class="vis" style="border: 1px solid #804000;" align="center" width="100%">
				<tr><th colspan="6">{$lang->get('Jogadores marcados')}:</th></tr>
				<tr class="nowrap">
